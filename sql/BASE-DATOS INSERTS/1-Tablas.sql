...
-- 🏢 Tabla MAE_PROPIETARIO  --NO USO ESTA TABLA ACTUALMENTE
...
GO
-- Crear la tabla con la restricción ajustada
...
-- Crear índice no agrupado para optimizar consultas por FECHA_LLEGADA y ESTADO
...
-- 👤 Tabla MAE_USUARIO_FASE (Relación Usuarios-Fases)
...
-- 🚪 Tabla MAE_FASE_PUERTA (Relación Fases-Puertas)
...
-- 📊 View for Users, Phases, and Door Access Report
...
GO
--Crear un índice compuesto para MAE_PERMISOS_DASHBOARD y otro para MAE_ELEMENTOS_DASHBOARD
CREATE NONCLUSTERED INDEX IDX_MAE_PERMISOS_DASHBOARD_TIPO_VISIBLE
ON MAE_PERMISOS_DASHBOARD(ID_TIPO_USUARIO, VISIBLE)
INCLUDE (ID_ELEMENTO);

--Solución: Crear índices para mejorar la búsqueda por ID_USUARIO
...
--El índice único ya cubre ID_USUARIO, pero incluir ESTADO puede mejorar el rendimiento.

CREATE NONCLUSTERED INDEX IDX_MAE_USUARIO_DEPARTAMENTO_ID_ESTADO
ON MAE_USUARIO_DEPARTAMENTO(ID_USUARIO, ESTADO)
INCLUDE (NRO_DPTO);

--El índice en ESTADO no incluye NRO_DPTO ni MONTO, lo que obliga a escanear más filas.

CREATE NONCLUSTERED INDEX IDX_MAE_DEUDOR_NRO_DPTO_ESTADO
ON MAE_DEUDOR(NRO_DPTO, ESTADO)
INCLUDE (MONTO);

--La ordenación por FECHA_CREACION y el filtro ESTADO = 1 pueden ser lentos sin un índice.
...
--Similar a noticias, el filtro y la ordenación necesitan un índice.
...
--El JOIN y los filtros en ESTADO, FECHA_ENTREGA, y ID_USUARIO necesitan optimización.
...
CREATE TABLE MAE_EVENTO (
    ...
    TITULO VARCHAR(100) NOT NULL,
    DESCRIPCION VARCHAR(255) NOT NULL,
    TIPO_EVENTO VARCHAR(50) NOT NULL CHECK (TIPO_EVENTO IN ('Reunión', 'Mantenimiento', 'Actividad Social', 'Inspección', 'Otro')),
...
CREATE NONCLUSTERED INDEX IDX_MAE_AVISO_ESTADO_FECHA
ON MAE_AVISO(ESTADO, FECHA_EXPIRACION, FECHA_PUBLICACION);
