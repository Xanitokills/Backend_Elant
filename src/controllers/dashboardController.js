...
const getDashboardData = async (req, res) => {
  ...
  try {
    ...
    const permissionsResult = await pool.request().query(`
      ...
      WHERE p.ID_TIPO_USUARIO IN (SELECT ID_TIPO_USUARIO FROM (VALUES ${userRoleIds.map(id => `(${id})`).join(",")}) AS Roles(ID_TIPO_USUARIO))
        AND e.ESTADO = 1
    ...
    const [
      ...
      pool.request().query(`
        ...
        WHERE a.ESTADO = 1 
          ...
          AND (
            ap.ID_TIPO_USUARIO IN (SELECT ID_TIPO_USUARIO FROM (VALUES ${userRoleIds.map(id => `(${id})`).join(",")}) AS Roles(ID_TIPO_USUARIO))
      ...
      pool.request().query(`
        ...
        WHERE e.ESTADO = 1 
          ...
          AND (
            ep.ID_TIPO_USUARIO IN (SELECT ID_TIPO_USUARIO FROM (VALUES ${userRoleIds.map(id => `(${id})`).join(",")}) AS Roles(ID_TIPO_USUARIO))
      ...
      pool.request().query(`
        ...
        WHERE d.ESTADO = 1
          AND (
            dp.ID_TIPO_USUARIO IN (SELECT ID_TIPO_USUARIO FROM (VALUES ${userRoleIds.map(id => `(${id})`).join(",")}) AS Roles(ID_TIPO_USUARIO))
