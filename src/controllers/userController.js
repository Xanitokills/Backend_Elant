...
const changePassword = async (req, res) => {
  ...
  try {
    ...
    // 1. Obtener el correo primero
    const resultCorreo = await pool.request()
      .input('ID_USUARIO', sql.Int, id)
      .query('SELECT CORREO FROM dbo.MAE_USUARIO WHERE ID_USUARIO = @ID_USUARIO');

    const correo = resultCorreo.recordset[0]?.CORREO;
    ...
    // 2. Enviar el correo
    const success = await sendResetPasswordEmail(correo, newPassword);
    if (!success) {
      return res.status(500).json({ message: 'Error al enviar el correo de restablecimiento' });
    }

    // 3. Solo si se envió, actualizamos la contraseña
    await pool.request()
      .input('ID_USUARIO', sql.Int, id)
